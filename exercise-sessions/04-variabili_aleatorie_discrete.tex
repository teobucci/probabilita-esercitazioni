...
\begin{equation*}
\EE[ X] =\sum\limits _{n=0}^{\infty }\mathbb{P}( X >n) .
\end{equation*}
...
\item Dato $k\in \{1,2,\dots \}$, quale $\lambda  >0$ massimizza $\mathbb{P}( X=k)$?
\end{enumerate}
...
\begin{equation*}
\EE[| h( x)| ] =\int _{\Omega }| h( X( \omega ))| d\mathbb{P}( \omega ) < +\infty \ \ \iff \ \ \int _{\mathbb{R}}| h( x)| P^{X}( dx) < +\infty 
\end{equation*}
...
Richiami Teoria
\begin{equation*}
\EE[ X] :=\int _{\Omega } Xd\mathbb{P} ,\ \ \ \ X:( \Omega ,\mathcal{A} ,\mathbb{P})\rightarrow ( E,\mathcal{E}) \ \text{VA}
\end{equation*}
...
Infatti:
\begin{equation*}
\EE[ X] =\underbrace{\int _{\Omega } X( \omega ) d\mathbb{P}( \omega ) =\int _{\mathbb{R}} xdP^{X}( x)}_{\text{Teorema "astratto-concreto"}}\overset{\text{discreta}}{=}\sum\limits _{x\in S} xp_{X}( x) =\sum\limits _{x\in S} x\mathbb{P}( X=x)
\end{equation*}
...
Se $X\in L^{2}(\mathbb{P})$, i.e. $\EE\left[ X^{2}\right] < +\infty $, allora\begin{equation*}
...
X_{i} \sim B( p) ,\forall i,\ \ \ \ \EE\left[ X_{i}^{2}\right] :=\sum\limits _{k\in \{0,1\}} k^{2}\mathbb{P}( X_{i} =k) =1\cdotp \mathbb{P}( X_{i} =1) =p
\end{equation*}

Poi\begin{equation*}
\begin{aligned}
\text{(b)} & =\EE\left[\sum\limits _{i< j} X_{i} X_{j}\right]\overset{\text{lin.}}{=}\sum\limits _{i< j} \EE[ X_{i} X_{j}]\overset{\text{(b1)}}{=}\sum\limits _{i< j}\mathbb{P}( E_{i} \cap E_{j})\\
...
\begin{aligned}
X_{i} ,X_{j} \sim B( p) \implies \EE[ X_{i} X_{j}] & =\sum\limits _{k\in \{0,1\}} k\mathbb{P}( X_{i} X_{j} =k)\\
...
\item \begin{equation*}
\begin{aligned}
\EE[ X] & :=\int _{\Omega } X( \omega ) d\mathbb{P}( \omega ) =\int _{\mathbb{R}} xdP^{X}( x) & \\
