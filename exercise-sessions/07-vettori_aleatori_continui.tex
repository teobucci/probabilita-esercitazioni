...
\newcommand{\SESOLOSE}{\ \Longleftrightarrow\ } % <=> con spazi
\newcommand{\SE}{\ \Longrightarrow\ } % => con spazi
...
\SE Var(M)&=\EE[M^2]-\EE[M]^2=4.029\cdot 10^{-6}
\end{aligned}
\end{gather*}
...
\begin{align*}
X&\sim\UUU\left(\left[0,\frac{d}{2}\right]\right) &\SE f_X(x)&=\frac{2}{d}\ \Ind_{\left[0,\frac{d}{2}\right]}(x) \\
\Theta&\sim\UUU\left(\left[0,\pi\right]\right) &\SE f_\Theta(\vartheta)&=\frac{1}{\pi}\ \Ind_{\left[0,\pi\right]}(\vartheta) \\
X&\indep\Theta&\SE f_{(X,\Theta)}(x,\vartheta)&=\frac{2}{\pi d}\ \Ind_{\left[0,\frac{d}{2}\right]}(x)\ \Ind_{\left[0,\pi\right]}(\vartheta)
\end{align*}
...
\[
\pi=2ap^{-1}=2a\displaystyle\lim_{t\to+\infty}\frac{t}{f}\SE \pi\approx 2a\frac{t}{f}
\]
...
\[
\begin{cases} u=x+1 \\ v=x+y-xy+x^2y  \end{cases}\SE \begin{cases} x=u-1 \\ y=\frac{v-x}{1-x+x^2}\end{cases}\SE g^{-1}(u,v)=\begin{pmatrix}
...
\SE \begin{cases}0\leq x\leq 1\SE 0\leq u-1\leq 1\SE 1\leq u \leq 2 \\ 0\leq y\leq 1\SE 0\leq \frac{v-u+1}{u^2-3u+3} \leq 1 \SE u-1\leq v\leq u^2-2u+2   \end{cases} \\
\SE g(S)=\{(u,v)\in\RR^2\ :\ 1\leq u \leq 2,\ u-1\leq v\leq u^2-2u+2 \}=Q\\
\, \\
\SE f_{(U,V)}(u,v)=\Ind_Q(u,v)\ \frac{1}{u^2-3u+3}
\end{gather*}
\fg{0.6}{7_20}
...
\SE F_T(t)=\left(1-e^{-(\lambda+\mu+\gamma)t} \right)\Ind_{[0,+\infty)}(t) \\
\SE T\sim\EEE(\lambda+\mu+\gamma)=\EEE(0.6)
\end{gather*}
...
\SE F_S(s)&=\left(1-e^{-0.6s}  \right)^2\Ind_{[0,+\infty)}(s)
\end{aligned}
\end{gather*}
...
\begin{gather*}
\begin{aligned}
u\in[0,1]&\SE f_{U}(u)=\intb f_{(U,V)}(u,v)\dv=\int_u^{1/u}\frac{1}{2v}\dv=-\log u \\
&\SE  f_{U}(u)=-\log u \ \Ind_{[0,1]}(u) \\
v\in[0,1]&\SE f_V(v)=\intb f_{(U,V)}(u,v)\du=\int_0^v \frac{1}{2v}\du=\frac{1}{2} \\
v\in[1,+\infty)&\SE f_V(v)=\intb f_{(U,V)}(u,v)\du=\int_1^{1/v} \frac{1}{2v}\du=\frac{1}{2v^2} \\
&\SE f_V(v)=\frac{1}{2}\ \Ind_{[0,1]}(v)+\frac{1}{2v^2}\ \Ind_{[1,+\infty)}(v)
\end{aligned}
\end{gather*}
...
\SE Cov(X,Y)&=\EE[XY]-\EE[X]\,\EE[Y]=2-\frac{9}{4}-\frac{1}{4}
\end{aligned}
\end{gather*}
