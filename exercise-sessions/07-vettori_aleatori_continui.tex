...
\begin{defn}[Domini normali regolari]$\\$
...
\begin{thm}[Di Jacobi per le variabili continue]$\\$
\label{introth3}
...
\begin{thm}[Di Jacobi per i vettori continui]$\\$
\label{introth4}
...
\begin{thm}[Calcolo delle leggi marginali]
\label{introth5-}
...
\begin{thm}[Fattorizzazione della legge congiunta]
\label{introth6}
$\\$$X\indep Y \iff \fXYxy=f_X(x)\cdot f_Y(y)$
\end{thm}

\begin{thm}[Condizione necessaria sui supporti per l'indipendenza]
\label{introth7}
...
\begin{thm}[Regola del valore atteso per vettori continui]
\label{introth8}
...
\begin{thm}[Risultato notevole (esercizio 4 del foglio 5)]
\label{introth9}
...
\begin{thm}[Coefficiente di correlazione unitario]
\label{introth10}
...
\begin{thm}[Alcune proprietà di valore atteso, varianza e covarianza]
\label{introth11}
...
\begin{thm}[Bilinearità della covarianza]
\label{introth12}
...
\begin{thm}[Valore atteso e varianza per trasformazioni affini]
\label{introth13}
...
\begin{thm}[Risultato notevole (esercizio 13 del foglio 7)]
\label{introth14}
...
\]
\end{thm}
\begin{thm}[Risultato notevole (esercizio 2 del foglio 7)]
\label{introth15}
...
\begin{rem}[Importanza storica dell'esperimento]$\\$
Consideriamo $l\leq d$. Abbiamo trovato che $p=\PP(A)=\dfrac{2l}{\pi d}$ e quindi
\[
\pi=\dfrac{2l}{p d}=2ap^{-1}
\]
con $a\coloneqq$ \event{rapporto tra lunghezza dell'ago e distanza tra le linee del pavimento}.
